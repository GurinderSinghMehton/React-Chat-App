...
const MultipleSelector = React.forwardRef(
  (
    ...
    const handleAdd = (item) => {
      ...
      const alreadyExist = selected?.find(
        (selected) => selected.value === item.value,
      );

      if (alreadyExist && alreadyExist.value !== "") {
        return toast.warning(`User "${item.label}" already selected`);
      }
    ...
    return (
      <div
        className={cn(
          "min-h-[50px] rounded-md border border-input px-6 py-6 text-sm flex flex-wrap gap-2 items-center relative",
          className,
        )}
        ref={containerRef}
        onClick={() => {
      ...
      >
        {/* Selected Items */}
        <div
          className={`flex flex-wrap gap-2 items-center overflow-y-auto ${showOptions ? "max-h-[40px]" : "max-h-[150px]"}`}
        >
          {selected.map((item) => (
            <Badge
              key={item.value}
        ...
        {showOptions && (
          <div className="h-[160px] p-4 overflow-y-auto w-full rounded-md bg-slate-200 absolute bottom-[-170px] left-0 flex flex-col gap-1">
            {options?.length > 0 &&
              options?.map((item, index) => (
                <div
                  className="p-2 rounded-lg shadow-inner text-black cursor-pointer border-2 border-transparent flex items-center hover:border-black"
                  onClick={() => handleAdd(item)}
                  style={
                    selected.some((selected) => selected.value === item.value)
            ...
            {options.length === 0 && (
              <p className="h-full w-full flex items-center justify-center text-md leading-10 text-gray-600">
                No results found.
              </p>
            )}
          </div>
